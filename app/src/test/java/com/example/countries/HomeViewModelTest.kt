...
class HomeViewModelTest {
    ...
    private val data = parseFileAsResponse<Countries>("CountriesFakeDataJson")
    ...
    private val mockErrorResponse = Response.error<Countries>(404,errorResponseBody)
    ...
    @Test
    fun `get countries from home viewModel and check is success`() = coroutineTestRule.runBlockingTest{
        whenever(apiService.getCountries(anyInt())).thenReturn(mockSuccessResponse)

        homeViewModel.getCountries(0).observeForever(countriesStateObserver)
    ...
    fun `get countries from home viewModel and check is unSuccess`() = coroutineTestRule.runBlockingTest{
        whenever(apiService.getCountries(anyInt())).thenReturn(mockErrorResponse)
